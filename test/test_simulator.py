...
def test_assign_order():
    current_orders_rows = []
    current_order_after_assign_rows = []
    ...
    for t in decision_epochs:
        ...
        # produce the current orders for viewing
        current_orders = [
            o
        ...
        # Produce the current orders after assignment for viewing
        current_orders_after_assign = [
            o
            for o in active_orders
            if o[0] <= t < o[2]
        ]
        for o in current_orders_after_assign:
            current_order_after_assign_rows.append(
                {
                    "time": t,
                    "release_time": o[0],
                    "ready_time": o[1],
    ...
    # More assert is needed
    # Save to CSV
    pd.DataFrame(current_orders_rows).to_csv("current_orders.csv", index=False)
    pd.DataFrame(current_order_after_assign_rows).to_csv("current_orders_after_assign.csv", index=False)
